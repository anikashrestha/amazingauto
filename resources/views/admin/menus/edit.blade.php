...
<div class="content-wrapper">
        ...
        <div class="panel-body">
            ...
            <input type="hidden" name="id" value="{{$menu->id}}">
                ...
                <select name="parent_id" id="parent_id" value="{{ $menu->parent_id}}" name="hidden" class="form-control">
                            <option hidden>0</option> 
                        @foreach($menus as $type)
                          
                                <option value="{{ $type->id }}" @if($type->id=== $menu->parent_id) selected='selected' @endif>{{ $type->title }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group {{ $errors->has('display_order')?'has-error':'' }}">
                    ...
                    <input type="number" class="form-control" name="display_order" id="display_order" value="{{ $menu->display_order }}">
                ...
                <div class="form-group {{ $errors->has('link')?'has-error':'' }}">
                    ...
                    <input type="text" class="form-control" name="link" id="link" value="{{ $menu->link}}">
                    @if($errors->has('link'))
                    <span class="help-block">
                        {{ $errors->first('link') }}
                    </span>
